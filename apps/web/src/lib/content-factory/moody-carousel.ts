...
const MEN_COVER_FAMILIES: { name: string; brief: string }[] = [
  ...
  },
  {
    name: "epic warrior",
    brief:
      "a lone armored warrior seen from a DISTANCE in an epic snowy atmosphere, hyperrealistic — a medieval knight, a spartan, a samurai, a viking, any legendary warrior in FULL armor (gleaming silver, burnished gold, or blackened steel). The shot is WIDE and cinematic: the warrior stands small-to-mid in an immense frozen landscape — a snowfield under a storm sky, a frozen mountain pass, a blizzard-swept ridgeline, drifting flakes catching what little light there is — NEVER close to the camera, NEVER a close-up or portrait framing. The atmosphere does the work: falling snow, blowing mist, storm light, scale. Face never visible — helmet on, visor down, or too distant to read.",
  },
];
...
SCENES: dark, dramatic, luxurious photography in FOUR families (nothing outside them): dark-luxury architecture (a black-glass penthouse tower with one lit floor, a cliff mansion above a storm sea, a marble entrance in near-darkness — the building shot low and dramatic, never a flat skyline), alpha wildlife (ONE alpha animal commanding an epic landscape — a wolf on a cracked frozen lake, a lion under lightning, a stag in blowing snow; the whole animal kingdom, never a recent post's animal), dark-luxury objects (a matte-black supercar under one cold spotlight, rain beading on a midnight sports car, a Swiss watch on black marble — one hero object, shot like a high-end ad), and epic warriors (a lone knight / spartan / samurai / viking in FULL silver-or-gold armor, seen from a DISTANCE in an epic snowy atmosphere — snowfield, frozen pass, blizzard ridgeline — wide cinematic framing, never close to the camera, face never visible). Desaturated, near-monochrome. Every frame DIM (white text must read on it). ANTI-BLAND RULE: every frame needs a clear dramatic SUBJECT with presence — never an empty flat landscape or bare horizon. NO people EVER except the distant-warrior carve-out (face never visible) and the lone animal, each only in its own family's scenes. UNLIMITED LIBRARY RULE: every example is a SEED, not a menu — INVENT a brand-new scene for every slide (new subject, location, season, weather, time, vantage) within these families; never render an example verbatim, never repeat a recent post's scene.
