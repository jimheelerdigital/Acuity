...
const REFLECTION_GUIDELINE = `- reflection: ONE short message written directly TO the user, second person, 2-3 sentences. This is the single most important field for how the product feels — treat it as such.
  ...
  WHAT IT IS NOT:
    ...
    * SAFETY: if the transcript contains self-harm, suicidal thoughts, abuse, or acute crisis, do NOT turn it into a neat observation or a pattern, and never reflect the distress back as an insight. Write one plain, gentle, non-clinical line that takes the person seriously — or if nothing safe can be said, reflect a small neutral detail from elsewhere in the entry instead. Never diagnose, advise, or dramatize.
