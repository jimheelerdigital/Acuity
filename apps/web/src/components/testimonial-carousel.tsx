...
export function buildTestimonialsWithLeader(testimonial: { quote: string; name: string; detail: string }): CarouselTestimonial[] {
  ...
  return [
    { quote: testimonial.quote, name: testimonial.name, role: testimonial.detail, initials, bgColor: "bg-[#8E6FE6]", imageSrc: pickFallbackHeadshot(testimonial.name) },
  ...
  return (
    <div className="w-[230px] sm:w-[260px] shrink-0 snap-start rounded-lg border border-white/10 bg-[#1E1C1A] p-3.5 sm:p-4">
      <div className="flex items-center gap-0.5 mb-2">
        {[...Array(5)].map((_, i) => (
          <svg key={i} className="h-3 w-3 text-[#8E6FE6]" fill="currentColor" viewBox="0 0 20 20">
