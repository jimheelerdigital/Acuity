...
export function ConsentGatedTrackers() {
  ...
  return (
    <>
      ...
      {/* Meta Pixel — marketing consent gated. Init + PageView fire
          inside the same Script so the load is atomic. Without
          consent: the Pixel never initialises and no events fire.
          The window.fbq stubs in signup / paywall code paths that
          call `fbq('track', ...)` no-op if fbq is undefined. */}
      {marketing && (
