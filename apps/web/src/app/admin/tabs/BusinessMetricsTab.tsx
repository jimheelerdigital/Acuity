...
export default function BusinessMetricsTab({
  ...
  return (
    <div className="space-y-8">
      ...
      {/* ── 1. Revenue Section ───────────────────────────────────── */}
      <section>
        <h2 className="mb-4 text-sm uppercase tracking-wider text-acuity-text-ter">
          Revenue
        </h2>
        ...
        <div className="mt-4 rounded-acuity-lg border border-acuity-card-border bg-acuity-card-bg shadow-acuity-soft px-5 py-3">
          <span className="text-sm text-acuity-text-ter">Paying users: </span>
          <span className="text-sm font-semibold text-acuity-text">
        ...
        <div className="mt-4">
          <ChartCard title="MRR Trend">
            {data.mrrTrend.length < 2 ? (
              <p className="py-12 text-center text-sm text-acuity-text-ter">Not enough data</p>
            ) : (() => {
                  ...
                  <div className="flex items-end gap-1 h-48 pt-4">
                    {data.mrrTrend.map((d: { month: string; mrr: number }, i: number) => (
                      <div key={i} className="flex-1 flex flex-col items-center justify-end" title={`${d.month}: ${fmtDollars(d.mrr)}`}>
                        <div className="w-full rounded-t bg-acuity-primary" style={{ height: `${Math.max(2, (d.mrr / max) * 100)}%` }} />
                      </div>
                    ))}
                  </div>
                );
      ...
      <section>
        ...
        <div className="mt-4 rounded-acuity-lg border border-acuity-card-border bg-acuity-card-bg shadow-acuity-soft p-5">
            ...
            <table className="w-full text-left text-sm">
              ...
              <tbody>
                {data.infraCosts.map((row, i) => (
                  <tr key={i} className="border-b border-acuity-line text-acuity-text-sec">
                    <td className="py-2 pr-4">{row.category}</td>
                    <td className="py-2 pr-4">{row.label}</td>
                    <td className="py-2 text-right tabular-nums">{fmtDollars(row.amountCents)}</td>
                  </tr>
                ))}
                <tr className="border-t border-acuity-line-strong font-medium text-acuity-text">
      ...
      <section>
        ...
        <div className="grid grid-cols-2 gap-4 lg:grid-cols-3">
          ...
          <MiniMetric
            ...
            color={data.cacCents === null ? "text-acuity-text-quiet" : undefined}
          />
          <MiniMetric
            label="Gross Margin / User"
            value={fmtDollars(data.grossMarginPerUserCents)}
            color={data.grossMarginPerUserCents >= 0 ? "text-acuity-good" : "text-acuity-bad"}
          />
          <MiniMetric
            label="Gross Margin %"
            value={fmtPct(data.grossMarginPct)}
            color={data.grossMarginPct >= 0 ? "text-acuity-good" : "text-acuity-bad"}
          />
          <MiniMetric
            label="LTV Estimate"
          ...
          <MiniMetric
            ...
            color={data.paybackMonths === null ? "text-acuity-text-quiet" : undefined}
          />
        </div>
      </section>
      ...
      <section>
        ...
        <div className="rounded-acuity-lg border border-acuity-card-border bg-acuity-card-bg shadow-acuity-soft p-6 text-center">
          <p className="text-sm text-acuity-text-ter mb-2">Net Profit / Loss This Month</p>
          <p
            className={`text-3xl font-bold tabular-nums ${
              isProfitable ? "text-acuity-good" : "text-acuity-bad"
            }`}
          >
            {isProfitable ? "+" : ""}
        ...
        <div className="mt-4">
          <ChartCard title="Revenue vs Costs vs Net">
            {data.profitTrend.length < 2 ? (
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left text-sm">
                  <thead><tr className="border-b border-acuity-line text-acuity-text-quiet text-xs">
                  ...
                  <tbody>
                    {data.profitTrend.map((d: { month: string; revenue: number; costs: number; net: number }, i: number) => (
                      <tr key={i} className="border-b border-acuity-line text-acuity-text-ter">
                        <td className="py-1.5 pr-3 text-xs">{d.month}</td>
                        <td className="py-1.5 pr-3 text-xs text-right text-acuity-good tabular-nums">{fmtDollars(d.revenue)}</td>
                        <td className="py-1.5 pr-3 text-xs text-right text-acuity-bad tabular-nums">{fmtDollars(d.costs)}</td>
                        <td className={`py-1.5 pr-3 text-xs text-right font-medium tabular-nums ${d.net >= 0 ? "text-acuity-good" : "text-acuity-bad"}`}>{fmtDollars(d.net)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
        ...
        <div className="mt-4 rounded-acuity-lg border border-acuity-card-border bg-acuity-card-bg shadow-acuity-soft p-5">
          <p className="text-sm text-acuity-text-ter">
            <span className="text-acuity-text font-medium">Break-even target:</span>{" "}
            Need{" "}
            <span className="text-acuity-text font-semibold tabular-nums">
        ...
        {!isProfitable && data.runwayMonths !== null && (
          <div className="mt-4 rounded-acuity-lg border border-acuity-bad bg-acuity-bad-soft p-5">
            <p className="text-sm text-acuity-bad">
              <span className="text-acuity-bad font-medium">Runway:</span>{" "}
              At current burn rate, runway is{" "}
              <span className="text-acuity-bad font-semibold tabular-nums">
        ...
        <div className="mt-4 rounded-acuity-lg border border-acuity-warn-soft bg-acuity-warn-soft p-4">
                    ...
                    return (
                      ...
                      >
                        ...
                        <td
                          ...
                          style={
                            stale
                              ? { color: "var(--acuity-bad)", fontWeight: 600 }
                              : undefined
                          }
                        >
                          {r.daysInactive ?? "—"}
...
function MiniMetric({
  ...
  return (
    <div className="rounded-lg bg-acuity-bg-inset px-4 py-3">
      <div className="text-[11px] uppercase tracking-wider text-acuity-text-quiet mb-1">
        {label}
      </div>
      <div className={`text-lg font-semibold tabular-nums ${color ?? "text-acuity-text"}`}>
        {value}
      </div>
      {sub && (
        <div className="text-[10px] text-acuity-text-quiet mt-0.5">{sub}</div>
      )}
    </div>
  );
}
