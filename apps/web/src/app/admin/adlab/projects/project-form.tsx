  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8">
      ...
      <Section title="Target Audience">
        <div className="grid gap-4 sm:grid-cols-2">
          <Field label="Min Age">
            <input type="number" value={data.targetAudience.ageMin}
              onChange={(e) => updateAudience("ageMin", Number(e.target.value))}
              className={inputClass} min={13} max={65} />
          </Field>
          <Field label="Max Age">
            <input type="number" value={data.targetAudience.ageMax}
              onChange={(e) => updateAudience("ageMax", Number(e.target.value))}
              className={inputClass} min={18} max={65} />
          </Field>
        </div>
        <TagField label="Geographies" tags={data.targetAudience.geo}
          inputValue={tagInputs.geo || ""} onInputChange={(v) => setTagInputs((t) => ({ ...t, geo: v }))}
          onAdd={() => addTag("geo")} onRemove={(i) => removeTag("geo", i)} placeholder="US, CA, UK..." />
        <TagField label="Interests" tags={data.targetAudience.interests}
          inputValue={tagInputs.interests || ""} onInputChange={(v) => setTagInputs((t) => ({ ...t, interests: v }))}
          onAdd={() => addTag("interests")} onRemove={(i) => removeTag("interests", i)} placeholder="productivity, journaling, mental health..." />
        <TagField label="Pain Points" tags={data.targetAudience.painPoints}
          inputValue={tagInputs.painPoints || ""} onInputChange={(v) => setTagInputs((t) => ({ ...t, painPoints: v }))}
          onAdd={() => addTag("painPoints")} onRemove={(i) => removeTag("painPoints", i)} placeholder="forgets tasks, no self-awareness..." />
        <TagField label="Desires" tags={data.targetAudience.desires}
          inputValue={tagInputs.desires || ""} onInputChange={(v) => setTagInputs((t) => ({ ...t, desires: v }))}
          onAdd={() => addTag("desires")} onRemove={(i) => removeTag("desires", i)} placeholder="clarity, better habits, weekly review..." />
        <TagField label="Identity Markers" tags={data.targetAudience.identityMarkers}
          inputValue={tagInputs.identityMarkers || ""} onInputChange={(v) => setTagInputs((t) => ({ ...t, identityMarkers: v }))}
          onAdd={() => addTag("identityMarkers")} onRemove={(i) => removeTag("identityMarkers", i)} placeholder="founders, ADHD, therapists..." />
      </Section>
      ...
      <Section title="USPs">
        {data.usps.map((usp, i) => (
          <div key={i} className="flex gap-2 mb-2">
            <input type="text" value={usp}
              onChange={(e) => { const next = [...data.usps]; next[i] = e.target.value; updateField("usps", next); }}
      ...
      <Section title="Banned Phrases">
        <TagField label="" tags={data.bannedPhrases}
          inputValue={tagInputs.bannedPhrases || ""} onInputChange={(v) => setTagInputs((t) => ({ ...t, bannedPhrases: v }))}
          onAdd={() => addTag("bannedPhrases")} onRemove={(i) => removeTag("bannedPhrases", i)} placeholder="unlock, elevate, game-changer..." />
      </Section>

      {/* Image & Creative */}
      <Section title="Image & Creative">
        <Field label="Image Style Prompt" error={errors.imageStylePrompt}>
          <textarea value={data.imageStylePrompt} onChange={(e) => updateField("imageStylePrompt", e.target.value)}
            className={`${inputClass} min-h-[80px]`} placeholder="Abstract, editorial style. Moody lighting, muted purple/indigo tones on dark background." />
        </Field>
      ...
      <Section title="Ad Configuration">
        <div className="grid gap-4 sm:grid-cols-2">
          <Field label="Target CPL ($)" error={errors.targetCplCents}>
            <input type="number" step="0.01" value={data.targetCplCents}
              onChange={(e) => updateField("targetCplCents", Number(e.target.value))} className={inputClass} placeholder="5.00" />
          </Field>
          <Field label="Daily Budget per Variant ($)" error={errors.dailyBudgetCentsPerVariant}>
            <input type="number" step="0.01" value={data.dailyBudgetCentsPerVariant}
              onChange={(e) => updateField("dailyBudgetCentsPerVariant", Number(e.target.value))} className={inputClass} placeholder="10.00" />
          </Field>
          <Field label="Test Duration (days)">
            <input type="number" value={data.testDurationDays}
              onChange={(e) => updateField("testDurationDays", Number(e.target.value))} className={inputClass} min={1} />
          </Field>
          <Field label="Conversion Objective">
            <select value={data.conversionObjective} onChange={(e) => updateField("conversionObjective", e.target.value)} className={inputClass}>
      ...
      <Section title="Meta Integration">
        <div className="grid gap-4 sm:grid-cols-2">
          <Field label="Ad Account ID" error={errors.metaAdAccountId}>
            <input type="text" value={data.metaAdAccountId} onChange={(e) => updateField("metaAdAccountId", e.target.value)}
              className={inputClass} placeholder="act_XXXXXXXXX" />
          </Field>
          <Field label="Pixel ID" error={errors.metaPixelId}>
            <input type="text" value={data.metaPixelId} onChange={(e) => updateField("metaPixelId", e.target.value)}
        ...
        <Field label="Conversion Event">
          <input type="text" value={data.conversionEvent} onChange={(e) => updateField("conversionEvent", e.target.value)}
            className={inputClass} placeholder="Lead, CompleteRegistration, Purchase..." />
        </Field>
      </Section>

      {/* Target Interests (Meta) */}
      <Section title="Target Interests (Meta)">
        <InterestSearch selected={data.targetInterests}
          onAdd={(interest) => { if (!data.targetInterests.find((i) => i.id === interest.id)) { updateField("targetInterests", [...data.targetInterests, interest]); } }}
          onRemove={(id) => { updateField("targetInterests", data.targetInterests.filter((i) => i.id !== id)); }} />
      </Section>
...
function TagField({ label, tags, inputValue, onInputChange, onAdd, onRemove, placeholder }: {
  label: string; tags: string[]; inputValue: string; onInputChange: (v: string) => void;
