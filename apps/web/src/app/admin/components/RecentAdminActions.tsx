...
export default function RecentAdminActions() {
  ...
  return (
    <div className="rounded-acuity-lg border border-acuity-card-border bg-acuity-card-bg shadow-acuity-soft" style={{ padding: 20 }}>
      <h3
        className="mb-4 font-mono text-[11px] font-bold uppercase tracking-[1.4px] text-acuity-text-ter"
      >
      ...
      {rows === null ? (
        ...
        <ul
          className="divide-y divide-acuity-line"
          style={{ fontSize: 14 }}
        >
          {rows.map((r) => (
            <li key={r.id} className="flex items-baseline justify-between gap-3 py-3">
              ...
              <div
                className="shrink-0 text-right text-acuity-text-ter tabular-nums"
                style={{ fontSize: 12 }}
              >
                <div>
                  {r.adminEmail ?? (
...
function ActionDetail({ row }: { row: Action }) {
  ...
  if (row.action === "user.send_magic_link") {
    return <span className="text-xs text-acuity-text-ter">to {target}</span>;
  }
