...
export function androidLaunch(v: AnnouncementVars): {
  ...
  const content = `
      ...
      ${para(greeting)}
      ${para(
        `Quick note: Acuity is finally on Android. If you\u2019ve been waiting for it \u2014 or just never got around to installing the app \u2014 you can get it on Google Play today.`
      )}
      ${para(
        `Here\u2019s all it is: you open Acuity and talk for a few minutes about whatever\u2019s in your head. It gives that back to you clearer than you\u2019d see it yourself \u2014 the tasks you mentioned in passing, the patterns you keep circling, and an honest read on where your energy is actually going.`
      )}
      <tr>
  ...
  return {
    ...
    html: trialLayout({
      ...
      preheader: "The Android app is here \u2014 get Acuity on Google Play.",
    }),
  };
}
