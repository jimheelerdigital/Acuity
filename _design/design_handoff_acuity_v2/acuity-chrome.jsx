// acuity-chrome.jsx — Acuity v2 chrome + reusable primitives.
// Sleek, dopamine-y. Includes ring/gauge/bar widgets used across screens.
...
const AcuityIcons = {
  ...
  pause: (p) => (
    <svg viewBox="0 0 24 24" width={p.size || 20} height={p.size || 20} fill={p.color}>
      <rect x="6.5" y="5" width="4" height="14" rx="1.5"/><rect x="13.5" y="5" width="4" height="14" rx="1.5"/>
    </svg>
  ),
  cog: (p) => (
    <svg viewBox="0 0 24 24" width={p.size || 18} height={p.size || 18} fill="none" stroke={p.color} strokeWidth={p.weight || 1.7} strokeLinecap="round" strokeLinejoin="round">
...
// Status bar
// ─────────────────────────────────────────────────────────────
function AcuityStatus({ dark = true, time = '7:42', tone }) {
...
// ─────────────────────────────────────────────────────────────
function AcuityTabBar({ active = 'home', t, withFab = true }) {
  const tabs = [
  ...
  return (
    <div style={{
      ...
      background: t.mode === 'dark'
        ? `linear-gradient(180deg, transparent 0%, ${t.bg}D9 70%, ${t.bg} 100%)`
        : `linear-gradient(180deg, transparent 0%, ${t.bg}E6 70%, ${t.bg} 100%)`,
    }}>
      <div style={{ position: 'relative', margin: '0 14px', pointerEvents: 'auto' }}>
          ...
          {tabs.map((tab) => {
            ...
            return (
              <div key={tab.id} style={{
                ...
                <tab.icon color={isActive ? t.primary : t.textTer} weight={isActive ? 2 : 1.7} />
                <div style={{
        ...
        {/* Center mic FAB — sits in the tasks slot, lifted */}
        {withFab && (
          <div style={{
...
// ─────────────────────────────────────────────────────────────
function AcuityThemePill({ label, hue, t, size = 'm' }) {
...
// ─────────────────────────────────────────────────────────────
function RingProgress({ value = 0, size = 96, stroke = 8, t, color, trackColor, children, gradient = true }) {
...
// ─────────────────────────────────────────────────────────────
function Sparkbar({ values, t, height = 28, color, gap = 3 }) {
...
// Device frame
// ─────────────────────────────────────────────────────────────
function AcuityDevice({ children, t, width = 402, height = 874 }) {
  return (
    <div style={{
      ...
      {/* Grain overlay \u2014 sits above bg, below content, no pointer events */}
      <div style={{
        position: 'absolute', inset: 0, zIndex: 1, pointerEvents: 'none',
        backgroundImage: t.grain, backgroundRepeat: 'repeat',
        mixBlendMode: t.mode === 'dark' ? 'overlay' : 'multiply',
      }} />
      <div style={{
...
Object.assign(window, {
  AcuityIcons, AcuityStatus, AcuityTabBar, AcuityThemePill,
  AcuityDevice, RingProgress, Sparkbar,
});
