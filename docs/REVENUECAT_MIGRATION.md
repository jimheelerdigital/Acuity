...
2. ~~**StoreKit version for observer mode.**~~ **Resolved 2026-08-27 — `STOREKIT_2` is correct, no change needed.** Verified against the installed `react-native-iap@15.3.1`: its podspec carries the comment *"React Native IAP uses StoreKit 2 via OpenIAP, which requires iOS 15+"* and pins `:ios => '15.0'`. A hard iOS 15 floor is a StoreKit-2-only requirement (StoreKit 1 has none), and the iOS sources import `OpenIAP` under `@available(iOS 15.0, ...)` with async `Task`-based transaction listeners. `configureRevenueCat`'s `STOREKIT_2` default therefore matches what our purchase library actually uses, and `EXPO_PUBLIC_RC_STOREKIT_VERSION` remains as the escape hatch if a future react-native-iap release changes this.
...
4. ~~**V2 product IDs are placeholders**~~ **Resolved 2026-08-23.** The V2 products exist in Apple, Google, Stripe and RevenueCat, and `pricing-plans.ts` carries the real ids (`PLACEHOLDER_V2_PRODUCT_IDS` is now `false`). This changed nothing for users — V2 is still selected only by `newPricingEnabled`, which remains off.
